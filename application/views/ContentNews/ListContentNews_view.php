...
<script>
	...
	function reset_search(){
		document.location = base_url+"content-news/reset-search";
	}
...
<?php 
	echo form_open('content-news/filter',array('id' => 'myform', 'class' => '')); 

	$data = $this->session->userdata('filter-ContentNews');
	...
	<div class="col-md-12">
		<div class="portlet box red">
			<div class="portlet-title">
				...
				<div class="actions">
					<a href="<?php echo base_url();?>content-news/add" class="btn btn-default btn-sm">
									...
									foreach($contentnews as $key => $val){
										echo"
											<tr>
												<td></td>
												<td>".$no."</td>
												<td>".tgltoview($val['news_date'])."</td>	
												<td>".$val['news_title']."</td>	
												<td>".$val['news_description']."</td>
												<td>
													<div class=\"fileinput-preview fileinput-exists thumbnail\" style=\"width: 200px; height: 150px; max-width: 200px; max-height: 150px;\">
														<img src=\"".base_url().'img/news/'.$val['news_image']."\" alt=\"\"/>
													</div>
												</td>
												<td>
													<a href='".$this->config->item('base_url').'content-news/edit/'.$val['news_id']."' class='btn default btn-xs purple'>
														<i class='fa fa-edit'></i> Edit
													</a>
													<a href='".$this->config->item('base_url').'content-news/delete/'.$val['news_id']."' onClick='javascript:return confirm(\"Are you sure you want to delete this entry ?\")' class='btn default btn-xs red'>
